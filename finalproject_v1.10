...
Authors: Derek Norton, Ryan Grimsley, Jordan Burns, Manu Redd, Priyatam Nuney
Version 1.10
*/
    ...
    for (size_t i = 0; i < userInput.length(); ++i) { //loop to iterate through user input
        if (userInput[i] == ' ') { //if statement to skip spaces
            ...
            if (userInput[i] == '+') { //checking '+' if it is a unary operator or addition operand, also checking for errors
                ...
                while (userInput[i] == ' '|userInput[i] == '+'| userInput[i] == '-'){ //loop that will erase spaces, jump excessive '+' operators, and change sign of next digit as needed
                    if (userInput[i] == ' '){ //erases spaces
                        userInput.erase(i, 1);
                    }else if (userInput[i] == '+'){ //skips excessive unary operators
                        i++;
                    }else if (userInput[i] == '-'){ //will flip boolean value that will change next digit to a negative number
                        i++;
                        if (isNegative == false){
                            isNegative = true;
                        }else{
        ...
        evaluated once the function within the parenthesis is completed*/
            ...
            i++;
            while (userInput[i] == '+'| userInput[i] == ' '| userInput[i] == '-'){ //loop that will erase spaces, jump excessive '+' operators, and change sign of next digit as needed
                if (userInput[i] == '+'){ //erases excessive unary operation
                    userInput.erase(i, 1);
                }else if (userInput[i] == ' '){ //erases spaces
                    userInput.erase(i, 1);
                }else if (userInput[i] == '-'){ //will flip sign of upcoming digit
                    i++;
                    if (isNegative == false){
                        isNegative = true;
                    }else{
            ...
            while (userInput[i] == ' '){
                ...
                if (userInput[i] == ' '){ //erases spaces
                    userInput.erase(i, 1);
                }else if (userInput[i] == '+'){ //jumps excessive unary operator
                    i++;
                }else if (userInput[i] == '-'){ //flips sign of upcoming digit as needed
                    i++;
                    if (isNegative == false){
                        isNegative = true;
                    }else{
            ...
            }if (userInput[i] == '(' & isNegative == true){ //will flip the sign of upcoming expression within parenthesis by implying -1 * (answer)
                nums.push(-1);
                ops.push('*');
                isNegative = false;
            }
